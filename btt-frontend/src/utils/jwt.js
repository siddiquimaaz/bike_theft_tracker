...
/** Return true if the access token expires in the next `bufferSecs` seconds */
export function isTokenExpiring(token, bufferSecs = 60) {
  const payload = decodeJWT(token);
  if (!payload?.exp) return true;
  return payload.exp - Date.now() / 1000 < bufferSecs;
}
